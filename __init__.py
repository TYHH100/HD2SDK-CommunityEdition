...
def InitializeConfig():
    global Global_gamepath, Global_searchpath, Global_tempdir, Global_configpath, Global_gamepathIsValid
    if os.path.exists(Global_configpath):
        ...
        try:
            ...
            Global_tempdir = config['DEFAULT'].get('tempdir', '')
        except:
            UpdateConfig()
...
def UpdateConfig():
    global Global_gamepath, Global_searchpath, Global_tempdir, Global_defaultgamepath, Global_gamepathIsValid
    if Global_gamepath == "":
        Global_gamepath = Global_defaultgamepath
    if Global_gamepathIsValid: 
        slim_init(Global_gamepath)
    config = configparser.ConfigParser()
    config['DEFAULT'] = {'filepath' : Global_gamepath, 'searchpath' : Global_searchpath, 'tempdir' : Global_tempdir}
    with open(Global_configpath, 'w') as configfile:
        config.write(configfile)

def GetTempDir():
    global Global_tempdir
    if Global_tempdir and os.path.exists(Global_tempdir):
...
class ChangeTempdirOperator(Operator, ImportHelper):
    ...
    def execute(self, context):
        global Global_tempdir
        Global_tempdir = self.filepath
        UpdateConfig()
        PrettyPrint(f"Changed Temporary Directory: {Global_tempdir}")
        return{'FINISHED'}
